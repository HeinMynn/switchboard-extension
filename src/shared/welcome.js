...
if (!firefox) {
  text('example-action', 'Switch');
  text('step-three', 'Save your work in this website’s tabs; they will refresh automatically. Choose Add account → Start new login, sign in, then return to Switchboard and click Save login.');
  text('routine-text', 'Save unfinished work, then choose Switch beside the account you want. Your existing website tabs reload using the saved login you chose. Check the account shown on the website. Click Save login again after signing in again and before quitting Chrome.');
  text('restore-text', 'If the selected login needs restoring, save unfinished work and choose Manage account → Restore saved login. Existing tabs refresh automatically. If a switch is interrupted, use Recover interrupted switch.');
}
if (firefox) {
  ...
  text('step-two', 'Choose a website, select Add account, and give it a name. Click Start new login, then sign in in the new container tab. Firefox keeps its login storage separate.');
  text('step-three-title', 'Add another account');
  text('step-three', 'Repeat Add account → Start new login with a different name. Sign in to your other account in that tab. You can keep both tabs open at the same time.');
  ...
  text('remove-text', 'Remove saved account and Remove website remove entries from Switchboard. Firefox containers and their cookies remain. Log out on the website to sign out; remove containers through Firefox to delete their stored data.');
