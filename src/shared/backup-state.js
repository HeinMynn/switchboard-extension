...
export function validateBackup(value, mode) {
  ...
  const { savedAccounts, connectedSites, lastSite, liveCookies } = value;
  if (!Array.isArray(savedAccounts) || savedAccounts.length > 1000 || !Array.isArray(connectedSites) || connectedSites.length > 1000 || !Array.isArray(liveCookies)) fail();
  ...
  for (const domain of connectedSites) {
    if (savedAccounts.filter(p => p.domain === domain && p.active).length > 1) fail();
    const names = savedAccounts.filter(p => p.domain === domain).map(p => p.name.toLowerCase());
...
export function backupOrigins(payload) {
  const domains = [...payload.connectedSites, ...(payload.browser === 'firefox'
    ? payload.savedAccounts.flatMap(p => p.cookies.map(c => c.domain.replace(/^\./, ''))) : [])];
  return [...new Set(domains.flatMap(origins))];
}

export async function gatherBackup(api, mode) {
  const state = await api.storage.local.get(['accountsState', 'savedAccounts', 'connectedSites', 'lastSite', 'backupRestoreJournal']);
  ...
  const payload = { format: 'switchboard', version: 1, browser: mode, createdAt: new Date().toISOString(), savedAccounts: [], connectedSites: [], lastSite: null, liveCookies: [] };
  if (mode === 'chrome') {
    ...
    for (const [domain, site] of Object.entries(state.accountsState?.sites || {})) {
      for (const a of site.accounts) payload.savedAccounts.push({ id: a.id, name: a.name, domain, savedAt: a.savedAt, active: a.id === site.active, cookies: a.cookies });
    ...
    payload.connectedSites = [...new Set([...(state.connectedSites || []), ...(state.savedAccounts || []).map(p => p.domain)])];
    const existing = new Set((await api.contextualIdentities.query({})).map(c => c.cookieStoreId));
    for (const p of state.savedAccounts || []) {
      ...
      payload.savedAccounts.push({ id: p.id, name: p.name, domain: p.domain, savedAt: null, active: false,
...
export async function restoreBackup(api, mode, payload, engine) {
  ...
  const state = await api.storage.local.get(['accountsState', 'savedAccounts', 'connectedSites', 'lastSite', 'pendingConnection', 'backupRestoreJournal']);
  ...
  const previous = { accountsState: state.accountsState || { sites: {}, pending: null }, savedAccounts: state.savedAccounts || [],
  ...
  try {
    if (mode === 'chrome') {
      ...
      for (const domain of payload.connectedSites) {
        sites[domain] = { active: payload.savedAccounts.find(p => p.domain === domain && p.active)?.id || null,
          accounts: payload.savedAccounts.filter(p => p.domain === domain).map(p => ({ id: p.id, name: p.name, savedAt: p.savedAt,
      ...
      await api.storage.local.set({ accountsState: { sites, pending: null }, savedAccounts: [], connectedSites: payload.connectedSites,
        lastSite: payload.lastSite, pendingConnection: null, backupRestoreJournal: null });
    } else {
      const mapping = new Map(), savedAccounts = [];
      for (const p of payload.savedAccounts) {
        ...
        savedAccounts.push({ id: container.cookieStoreId, name: p.name, domain: p.domain });
      }
      for (const p of payload.savedAccounts) for (const c of p.cookies) {
      ...
      await api.storage.local.set({ savedAccounts, connectedSites: payload.connectedSites, lastSite: payload.lastSite, backupRestoreJournal: null });
    }
  } catch (error) {
