...
const payload = browser => ({ format: 'switchboard', version: 1, browser, connectedSites: ['example.com'], lastSite: 'example.com',
  pockets: [{ id: 'old-store', name: 'Work', domain: 'example.com', active: browser === 'chrome', savedAt: null, cookies: [cookie()] }],
...
test('validation rejects foreign cookie scopes, duplicate identities, invalid dates and cross-browser imports', () => {
  assert.equal(validateBackup(payload('chrome'), 'chrome').pockets.length, 1);
  const www = payload('chrome'); www.pockets[0].cookies[0] = cookie('www', { domain: 'www.example.com', hostOnly: true });
  assert.doesNotThrow(() => validateBackup(www, 'chrome'));
  for (const mutate of [p => p.pockets[0].cookies[0].domain = '.unrelated.com',
    p => p.pockets.push(p.pockets[0]), p => p.pockets[0].cookies[0].path = 'bad',
    p => { p.pockets[0].cookies[0].session = false; }, p => p.lastSite = 'other.com']) {
...
function fixture(mode) {
  let data = { lastSite: 'previous.com', connectedSites: ['previous.com'], pockets: [] };
...
test('Firefox remaps container IDs and cookie destinations without deleting old containers', async () => {
  ...
  imported.pockets[0].cookies.push(cookie('provider', { domain: 'login.provider.com', hostOnly: true }),
    cookie('expired', { session: false, expirationDate: 1 }));
  await restoreBackup(f.api, 'firefox', imported);
  assert.equal(f.data().pockets[0].id, 'new-1');
...
test('Firefox export reads each saved container including external sign-in cookies', async () => {
  const f = fixture('firefox');
  await f.api.storage.local.set({ pockets: [{ id: 'existing', domain: 'example.com', name: 'Personal' }], connectedSites: ['example.com'], lastSite: 'example.com' });
  ...
  assert.equal(result.pockets[0].cookies[0].domain, '.provider.com');
});
...
test('denied permissions and malformed imports leave configuration untouched', async () => {
  ...
  const invalid = payload('chrome'); invalid.pockets[0].cookies[0].domain = '.elsewhere.com';
...
test('Chrome restores distinct saved and live cookies and remaps cookie store IDs', async () => {
  ...
  assert.equal(exported.pockets[0].cookies[0].value, 'secret');
